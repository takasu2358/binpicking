...
# ply_filepath = data_folder_path / "ply" / "error" / "choose_connect.ply"
ply_filepath = data_folder_path / "ply" / "2022-9-1" / "U-shape" / "U-1.ply"
# ply_filepath = data_folder_path / "ply" / "2022-9-1" / "ko-shape" / "ko-5.ply"
    ...
    def cut_branch(self, skeleton, ad_branch_point, branch_point, branch_end_point):
        ...
        while len(branch_point) > 0 or len(ad_bra) > 0:
            ...
            if len(ad_bra) > 0:
                ...
                skeleton, branch_point, ad_bra, ad_branch_point, last_line_list, _, interpolate_line_list = self.ad_cutting(skeleton, xy, branch_point, ad_bra, ad_branch_point, branch_end_point, last_line_list, interpolate_line_list) #ad_cuttingで3点連結の分岐点の場合の枝切り
...
class ConnectLine():
            ...
            for switch in range(0, 2):
                ...
                for i in sorted_CScosts:
                    ...
                    if vs_count in correct_count_index_list:
                        continue
                    if [count, vs_count] in count_list or [vs_count, count] in count_list:
                        continue
                    ...
                    flag, _ = self.cal_dif_curvature(curvatures, line, interpolate, 1)
                    check = 0
                    if flag == 1:
    ...
    def cal_curvature_of_connected_several_lines(self, skel_list, explored, count_list, count_list_index, interpolate_list, end_count, current_count_list_index):
        ...
        interpolate = []
        for i, linter in enumerate(line_separate):
            if i % 2 == 1:
                interpolate.extend(linter)
        flag, value = self.cal_dif_curvature(curvature, line, interpolate, 1)
        ...
        if flag == 1:
            ...
            interpolate0 = []
            for i, linter in enumerate(line_separate_copy):
                if i % 2 == 1:
                    interpolate0.extend(linter)
            curvature0 = self.calc_curvature(line0)
            flag, value0 = self.cal_dif_curvature(curvature0, line0, interpolate0, 1)
            ...
            for i, linter in enumerate(line_separate_copy):
                if i % 2 == 1:
                    interpolate1.extend(linter)
            curvature1 = self.calc_curvature(line1)
            flag, value1 = self.cal_dif_curvature(curvature1, line1, interpolate1, 1)
    ...
    def cal_dif_curvature(self, curvature2, line, interpolate, flag=0):
        V = Visualize()
        ...
        #     MI = MakeImage()
        #     limg = MI.make_colorimage(line, 255)
        #     for xy in interpolate:
        #         limg[xy[0]][xy[1]] = [255, 0, 0]
    ...
    #     curvatures = []
    #     num = 1
    #     for i in np.arange(self.interval, length-self.interval, self.interval):
    #         dxn = line[i][0] - line[i - self.interval][0]
    #         dxp = line[i + self.interval][0] - line[i][0]
    #         dyn = line[i][1] - line[i - self.interval][1]
    #         dyp = line[i + self.interval][1] - line[i][1]
    ...
    #     if length - (i+self.interval) >= int(self.interval*3/4):
    #         i += self.interval
    #         dxn = line[i][0] - line[i - self.interval][0]
    #         dxp = line[-1][0] - line[i][0]
    #         dyn = line[i][1] - line[i - self.interval][1]
    #         dyp = line[-1][1] - line[i][1]
    #         thetaOA = math.degrees(math.atan2(dyn, dxn))
    #         thetaOB = math.degrees(math.atan2(dyp, dxp))
    #         dif_theta = thetaOB - thetaOA
    #         if dif_theta > 180:
    ...
    #     # curvatures = [abs(value) for value in curvatures]
        ...
        for count in range(0, line_num-1):
            ...
            for switch in (0, 1):
                ...
                for i in sorted_CScosts:
                    ...
                    flag, _ = CL.cal_dif_curvature(curvatures, line, interpolate)
                    check = 0
                    if flag == 1:
                        explored = [count, vs_count]
                        flat_count_copy = flat_count.copy()
   ...
   #########################################################################################################
    ...
    def check_connect(self, combi, side, pos, line_list):
        ...
        for x in pos:
            ...
            [flat_num.append(combi_copy[num]) for num in x]
            ...
            for _ in range(len(x)):
                index = [elem for elem in x if region_num in combi_copy[elem]][0]
                if region_num == combi_copy[index][0]:
                    series_side.append(side[index])
                else:
                    side[index].reverse()
                    series_side.append(side[index])
                next_region_num = [elem for elem in combi_copy[index] if not elem == region_num][0]
        ...
        for combi_elem, side_elem in zip(combi_copy, side):
            if combi_elem == []:
                continue
    ...
    def graspability(self, poi, next_poi, prev_poi, optimal_grasp, Hc_original_list, depth, z, index, contact_image):
        ...
        # for angle in np.arange(0, 10, 22.5):
            ...
            for i, Hc_rotate in enumerate(Hc_rotate_list):
                ...
                # CI_crop_color = cv2.circle(CI_crop_color, (int(self.cutsize), int(self.cutsize)), 3, [0, 0, 255], -1)
